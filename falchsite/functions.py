...
def error_list(request):
    ...
    logs = Log.objects.all().order_by('-timestamp').values('data', 'timestamp', 'signal', 'notification', 'device')[:100]
    errors = list(logs)
 ...
 #               - int(l.get("data")))>50:
        ...
        if l.get("signal") == 6:
            ...
            if float(l.get("data")) < 93 and float(l.get("data")) > 40:
                errors.remove(l)

        if l.get("signal") == 10:
            # Signal ID 10
            # Berechnet, ob Aufheitzeit lang genug
            if int(l.get("data")) > 110:
                errors.remove(l)
